...
# Tabela de caracteres desenhados segundo a fonte 8x8 pixels do jogo The Way of the Exploding Fist
# Não foram desenhados caracteres que não estão presentes no jogo, esses estão seguindo a fonte 8x8 pixels do ZX-Spectrum
GameTabChar:
.word 	
	0x00000000, 0x00000000, 0x10101010, 0x00100010, 0x00002828, 0x00000000, 0x28FE2828, 0x002828FE, 
	0x38503C10, 0x00107814, 0x10686400, 0x00000000, 0x28102818, 0x003A4446, 0x00001010, 0x00000000, 
	0x20201008, 0x00081020, 0x08081020, 0x00201008, 0x38549210, 0x00109254, 0xFE101010, 0x00101010, 
	0x00000000, 0x10081818, 0xFE000000, 0x00000000, 0x00000000, 0x18180000, 0x10080402, 0x00804020, 
	
	# Números
	0x7E6E663C, 0x003C6676, 0x18381818, 0x007E1818, 0x0C06663C, 0x007E3018, 0x1C06663C, 0x003C6606, # 0 - 3
	0x6C3C1C0C, 0x000C0C7E, 0x067C607E, 0x003C6606, 0x7C60301C, 0x003C6666, 0x180C067E, 0x00303030, # 4 - 7
	0x3C66663C, 0x003C6666, 0x3E66663C, 0x00380C06, 												# 8 - 9
	
	0x7E66663C, 0x00666666, 0x00181800, 0x10081818, 0x6060663C, 0x003C6660, 0x00FE0000, 0x000000FE,
	0x04081020, 0x00201008, 0x08044438, 0x00100010, 0x545C4438, 0x0038405C, 
	
	# Letras Maiúsculas
	0x7E66663C, 0x00666666, 0x7C66667C, 0x007C6666, 0x6060663C, 0x003C6660, 0x66666C78, 0x00786C66, # A - D
	0x7C60607E, 0x007E6060, 0x7C40407C, 0x00404040, 0x5C404438, 0x00384444, 0x7E666666, 0x00666666, # E - H
	0x1818187E, 0x007E1818, 0x0C0C0C3E, 0x00386C0C, 0x70786C66, 0x00666C78, 0x60606060, 0x007E6060, # I - L
	0x6B7F7763, 0x0063636B, 0x7E7E7666, 0x0066666E, 0x6666663C, 0x003C6666, 0x7C66667C, 0x00606060, # M - P
	0x44444438, 0x0C384444, 0x7C66667C, 0x0066666C, 0x3C60663C, 0x003C6606, 0x1818187E, 0x00181818, # Q - T
	0x44444444, 0x00384444, 0x66666666, 0x00183C66, 0x54828282, 0x00282854, 0x10284444, 0x00444428, # U - X
	0x3C666666, 0x00181818, 0x1008047C, 0x007C4020, 												# Y - Z
	
	0x20202038, 0x00382020, 0x10204080, 0x00020408, 0x08080838, 0x00380808, 0x00442810, 0x00000000, 
	0x00000000, 0xFE000000, 0x00000810, 0x00000000, 0x3C043800, 0x003A4444, 0x24382020, 0x00582424, 
	0x201C0000, 0x001C2020, 0x48380808, 0x00344848, 0x44380000, 0x0038407C, 0x70202418, 0x00202020, 
	0x443A0000, 0x38043C44, 0x64584040, 0x00444444, 0x10001000, 0x00101010, 0x10001000, 0x60101010, 
	0x28242020, 0x00242830, 0x08080818, 0x00080808, 0x49B60000, 0x00414149, 0x24580000, 0x00242424, 
	0x44380000, 0x00384444, 0x24580000, 0x20203824, 0x48340000, 0x08083848, 0x302C0000, 0x00202020, 
	0x201C0000, 0x00380418, 0x10381000, 0x00101010, 0x48480000, 0x00344848, 0x44440000, 0x00102844, 
	0x82820000, 0x0044AA92, 0x28440000, 0x00442810, 0x24240000, 0x38041C24, 0x043C0000, 0x003C1008, 
	0x2010100C, 0x000C1010, 0x10101010, 0x00101010, 0x04080830, 0x00300808, 0x92600000, 0x0000000C, 
	0x243C1818, 0xA55A7E3C, 0x99FF5A81, 0x99663CFF, 0x10280000, 0x00000028, 0x10081020, 0x00081020
...
ecallException:   addi    sp, sp, -264              # Salva todos os registradores na pilha
 ...
 ecallException.pula:   
    ...
    beq     t0, a7, goToPrintInt
	addi    t0, zero, 301             # ecall 1 = print int
    beq     t0, a7, goToPrintIntGame
    ...
    beq     t0, a7, goToPrintString
	addi    t0, zero, 304             # ecall 4 = print string
    beq     t0, a7, goToPrintStringGame
    ...
    beq     t0, a7, goToPrintChar
	addi    t0, zero, 311             # ecall 11 = print char
    beq     t0, a7, goToPrintCharGame
	...
	goToPrintIntGame:	jal     printIntGame               	# chama printInt
			j       endEcall

	goToPrintString: jal     printString           	# chama printString
			 j       endEcall

	goToPrintStringGame: jal     printStringGame           	# chama printString
			 j       endEcall

	goToPrintChar:	jal     printChar		# chama printChar
			j       endEcall

	goToPrintCharGame:	jal     printCharGame		# chama printChar
			j       endEcall
...
####################################################################################################
# ECALLs modificadas seguindo a fonte 8x8 pixels do jogo The Way of the Exploding Fist
# 3XX ( 301 - PrintIntGame, 304 - PrintStringGame, 311 - PrintCharGame )
...
printIntGame:	
		...
		bge a0, zero, ehposprintIntGame		# Se eh positvo
...
loop1printIntGame:	
		TEM_M(s8,printIntGame.pula1)
		DIV10(t4,a0)
		REM10(t3,a0)
		j printIntGame.pula1d
printIntGame.pula1:	
...
printIntGame.pula1d:
		...
		bne 	a0, zero, loop1printIntGame		# verifica se o numero eh zero
...
loop2printIntGame:	
		...
		bne 	t1, zero, loop2printIntGame		# eh o ultimo?
		sb 	zero, 0(t0)			# insere \NULL na string
		
		la 	a0, TempBuffer		# Endereco do buffer da srting
		jal 	printStringGame		# chama o print string
				
		lw 	ra, 0(sp)			# recupera a
		addi 	sp, sp, 4		# libera espaco

fimprintIntGame: ret				# retorna
...
loopprintStringGame:
    	...
    	beq     a0, zero, fimloopprintStringGame	# string ASCIIZ termina com NULL
    	jal     printCharGame       		# imprime char
		addi    a1, a1, 8                 	# incrementa a coluna
		li 	t6, 313		
		blt	a1, t6, NaoPulaLinhaGame	    # se ainda tiver lugar na linha
    	addi    a2, a2, 8                 	# incrementa a linha
    	mv    	a1, zero					# volta a coluna zero

NaoPulaLinhaGame:	
		addi    s0, s0, 1					# proximo caractere
    	j       loopprintStringGame       	# volta ao loop

fimloopprintStringGame:	
		lw      ra, 0(sp)    	# recupera ra
		lw 	s0, 0(sp)			# recupera s0 original
    	addi    sp, sp, 8		# libera espaco

fimprintStringGame:	ret      	    # retorna
...
printCharGame:	
		...
		li 	tp, ' '
		blt 	a0, tp, printCharGame.NAOIMPRIMIVEL	# ascii menor que 32 nao eh imprimivel
		li 	tp, '~'
		bgt	a0, tp, printCharGame.NAOIMPRIMIVEL	# ascii Maior que 126  nao eh imprimivel
    	j       printCharGame.IMPRIMIVEL
    
printCharGame.NAOIMPRIMIVEL: li      a0, 32		# Imprime espaco

printCharGame.IMPRIMIVEL:	
		li	tp, NUMCOLUNAS		# Num colunas 320
		TEM_M(s8,printCharGame.mul1)
		MULTIPLY(t4,tp,a2)
		j printCharGame.mul1d

printCharGame.mul1:		mul     t4, tp, a2			# multiplica a2x320  t4 = coordenada y

printCharGame.mul1d:	
		add     t4, t4, a1               	# t4 = 320*y + x
		addi    t4, t4, 7                 	# t4 = 320*y + (x+7)
		li      tp, VGAADDRESSINI0          	# Endereco de inicio da memoria VGA0
		beq 	a4, zero, printCharGame.PULAFRAME		# Verifica qual o frame a ser usado em a4
		li      tp, VGAADDRESSINI1          	# Endereco de inicio da memoria VGA1

printCharGame.PULAFRAME:	
		...
		la      t3, GameTabChar		# endereco dos caracteres na memoria
		add     t2, t2, t3               	# endereco do caractere na memoria
		lw      t3, 0(t2)                 	# carrega a primeira word do char
		li 	t0, 4				# i=4

printCharGame.forChar1I:	
		beq     t0, zero, printCharGame.endForChar1I # if(i == 0) end for i
    	addi    t1, zero, 8               	# j = 8

printCharGame.forChar1J:	
		beq     t1, zero, printCharGame.endForChar1J # if(j == 0) end for j
        andi    s9, t3, 0x001			# primeiro bit do caracter
        srli    t3, t3, 1             		# retira o primeiro bit
        beq     s9, zero, printCharGame.printCharGamePixelbg1	# pixel eh fundo?
        sb      t6, 0(t4)             		# imprime pixel com cor de frente
    	j       printCharGame.endCharPixel1

printCharGame.printCharGamePixelbg1:	sb      t5, 0(t4)                # imprime pixel com cor de fundo

printCharGame.endCharPixel1: 
		addi    t1, t1, -1                	# j--
		addi    t4, t4, -1                	# t4 aponta um pixel para a esquerda
		j       printCharGame.forChar1J		# vollta novo pixel

printCharGame.endForChar1J: 
		addi    t0, t0, -1 		# i--
    	addi    t4, t4, 328           	# 2**12 + 8
    	j       printCharGame.forChar1I	# volta ao loop

printCharGame.endForChar1I:	
		lw      t3, 4(t2)           	# carrega a segunda word do char
		li 	t0, 4			# i = 4
printCharGame.forChar2I:    
		beq     t0, zero, printCharGame.endForChar2I  # if(i == 0) end for i
    	addi    t1, zero, 8             # j = 8

printCharGame.forChar2J:	
		beq	t1, zero, printCharGame.endForChar2J # if(j == 0) end for j
       	andi    s9, t3, 0x001	    		# pixel a ser impresso
    	srli    t3, t3, 1                 	# desloca para o proximo
		beq     s9, zero, printCharGame.printCharGamePixelbg2 # pixel eh fundo?
		sb      t6, 0(t4)			# imprime cor frente
		j       printCharGame.endCharPixel2		# volta ao loop

printCharGame.printCharGamePixelbg2:	sb      t5, 0(t4)		# imprime cor de fundo

printCharGame.endCharPixel2:	
		addi    t1, t1, -1		# j--
    	addi    t4, t4, -1              # t4 aponta um pixel para a esquerda
    	j       printCharGame.forChar2J

printCharGame.endForChar2J:	
		addi	t0, t0, -1 		# i--
		addi    t4, t4, 328		#
    	j       printCharGame.forChar2I	# volta ao loop

printCharGame.endForChar2I:	ret				# retorna
